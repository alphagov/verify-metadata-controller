...
When("I run the java executable with some appropriate parameters") do
  @last_output = run_app("connector", "rsa")
end
...
def run_app(node_type, algorithm)
  `java -classpath '/opt/cloudhsm/java/*:mdgen/lib/*' uk.gov.ida.mdgen.MetadataGenerator #{node_type} ../test/#{node_type}.yml ../test/cloudhsmkeycert.rsa.pem --supplied-saml-signing-cert-file ../test/cloudhsmkeycert.rsa.pem --supplied-saml-encryption-cert-file ../test/cloudhsmkeycert.rsa.pem --algorithm #{algorithm} --hsm-metadata-signing-label this-is-a-cloudhsmtool-thing --output ./metadata.xml 2>&1`
end
